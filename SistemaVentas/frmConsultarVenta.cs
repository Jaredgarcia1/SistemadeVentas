...
namespace SistemaVentas
{
    public partial class frmConsultarVenta : Form
    {
        ...
        private void CargarDatos()
        {
            ...
            List <Venta> oListaVenta = CdVenta.ObtenerListaVenta(txtCodigoVenta.Text.Trim(), dtpFechaInicio.Value, dtpFechaFin.Value,txtDocumentoCliente.Text.Trim(),txtNombreCliente.Text.Trim());
            if (oListaVenta.Count > 0)
            {
                ...
                {
                    tabla.Rows.Add(row.IdVenta, row.TipoDocumento, row.Codigo, row.VFechaRegistro.ToString("dd/MM/yyyy"), row.Cliente.NumeroDocumento, row.Cliente.Nombre, row.TotalCosto);
                };
