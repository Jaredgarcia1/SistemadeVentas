    ...
    public partial class frmProducto : Form
    {
        ...
        private void CargarDatos()
        {
            ...
            if (oListaProducto.Count > 0  && oListaProducto != null)
            {
                ...
                {
                    tablaRol.Rows.Add(row.IdProducto, row.Codigo, row.ValorCodigo, row.Nombre, row.Descripcion, row.IdCategoria, row.Activo,row.Categoria.Descripcion, row.Activo == true ? "Activo" : "No Activo");
                }
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (dgvProducto.SelectedRows.Count > 0)
            {
                ...
                if (MessageBox.Show(string.Format("{0} '{1}' {2}", "¿Desea eliminar el producto", NombreProducto, "permanentemente?"), "Mensaje", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    bool Respuesta = CdProducto.EliminarProducto(IdProducto);
                    if (Respuesta)
                    {
