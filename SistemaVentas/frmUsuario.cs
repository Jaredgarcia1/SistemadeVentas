...
namespace SistemaVentas
{
    public partial class frmUsuario : Form
    {
        ...
        private void CargarDatos()
        {
            ...
            List<Usuario> oListaUsuario = CdUsuario.ObtenerUsuarios();
            if (oListaUsuario.Count > 0)
            {
                ...
                {
                    tablaUsuario.Rows.Add(row.IdUsuario, row.Rol.Descripcion, row.NombreUsuario, row.Nombres, row.Apellidos, row.Correo,row.Activo == true ?"Activo":"No Activo" , row.Clave, row.IdTienda, row.IdRol,row.Activo);
                }
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            if (dgvUsuarios.SelectedRows.Count > 0)
            {
                ...
                {
                    bool Respuesta = CdUsuario.EliminarUsuario(IdUsuario);
                    if (Respuesta)
                    {
                    ...
                    {
                        MessageBox.Show(string.Format("{0} '{1}' {2} \n{3}", "El usuario", NombreUsuario, "no fue eliminado.", "El usuario se encuentra activo, tiene que cambiarlo a inactivo primero"), "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                }
            }
            else
