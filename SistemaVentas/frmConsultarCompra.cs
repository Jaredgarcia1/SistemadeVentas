...
namespace SistemaVentas
{
    public partial class frmConsultarCompra : Form
    {
        ...
        private void CargarDatos()
        {
            ...
            List<Compra> oListaCompra = CdCompra.ObtenerListaCompra(dtpFechaInicio.Value, dtpFechaFin.Value,int.Parse(((ComboBoxItem)cboProveedor.SelectedItem).Value.ToString()) , int.Parse(((ComboBoxItem)cboTienda.SelectedItem).Value.ToString()));
            if (oListaCompra.Count > 0)
            {
                ...
                {
                    tabla.Rows.Add(row.IdCompra, row.NumeroCompra,row.Proveedor.RazonSocial,row.Tienda.Nombre, row.FechaCompra, row.TotalCosto);
                };
